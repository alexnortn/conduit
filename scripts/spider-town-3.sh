...
do
	...
	wget --spider --recursive --level=inf --no-verbose --show-progress --wait=0.25 --random-wait=on -c -e robots=off -U mozilla --no-cookies --reject=.rss,.gif,.png,.jpg,.css,.js,.txt --reject-regex='\?' --directory-prefix=$baseDir --output-file=$linklist "$url"
